...
<br/>
데이터 모델을 짜는 데는 다양한 명령어가 개입했다. 솔직히 여기서 쫄리는 사람 많을 거 같다. 내가 그랬으니까ㅎ 가령 `@map, @@map`은 스키마 파일과 DB의 대소문자 불균형 문제를 해결하는 데 쓰이고, 관계형 같은 경우 `@relation(fields:[필드명], refrerences: [참조값])`의 형태로 사용되고 있었다.  
