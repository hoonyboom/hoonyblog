...
<br/>
여담으로 이건 타입스크립트 처음 쓸 때 말했던 거지만ㅋㅋ   
타입에서 `extends`는 "물려받으면 또는 확장하면" 이라는 말보단 `이꼴(=), T가 무엇무엇이면~`으로 얘기됐을 때 이해가 쉬워지는 거 같다. 내 모국어가 한국어인지라 이렇게 얘기해야 머리가 빨리 돈다.. (<Lnk text='링크' href='https://hyezoprk.com/posts/Typescript-Omit#제네릭이-어떻게-짜여있길래' />)