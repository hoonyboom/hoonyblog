...
그렇게 보면 두번째도 같은 맥락이다. 타입스크립트의 소극성이라고 불러도 될까? 타입스크립트가 방파제 역할만 할 게 아니라, 파싱, 나아가 가공까지 할 수 있으면 좋겠단 생각을 종종 했다. 내가 기여한 바는 없지만 말이다.    
<br/>
...
### zod
타입스크립트의 스키마 선언 그리고 검증을 도와주는 라이브러리다. 
...
여기서는 최대한 간단하게 `name`만을 추출해봤다. 그러면 각 인물들의 이름만 속속들이 `fetch` 될 거라 예상되는데,
