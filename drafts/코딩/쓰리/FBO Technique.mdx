...
### 무슨 뜻이죠
이유 없는 준말은 없겠지만 처음에 이 용어를 듣곤 멍때렸다. FBO? FBI, UFO 이딴 것만 생각났지 아무 감도 안 왔다. 알고 보면 의미가 고스란히 드러나는 단어인데 말이다.
<br/>
`FBO`는 `Frame Buffer Object`[^1]의 약자다. 아니, 풀어봐도 모르겠는데? 사실 나는 그랬지만 버퍼링의 뜻을 떠올리면 어려운 것만도 아니다. 다시 보니 왠지 프레임의 속도(연산) 차를 조정하기 위해 고안된 어떤 객체일 것만 같은데, 정말 그럴까? 맞다 👀  
