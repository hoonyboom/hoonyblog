...
<br/>
그런 관점에서 `Three.js`는 맥락에서 벗어난다. `Three.js`로 얻는 건 즐거움 또는 놀라움이지 사용성, 편의성은 아니기 때문이다. 그럼에도 불구하고 이끌리는 건 왤까? 특별하고 힙해서? 꼭 나쁜 이유만은 아닌 것 같다. 동기야 뭐든 웹을 도화지 삼는 사람에게 표현의 스펙트럼을 넓히는 건 권장할 만한 일 아닌가.
<br/>
나는 그래서 공부했다. 이어지는 일련의 노트는 `Yuri`의 GPGPU 강의 정리본이다. 러시아 사람의 어눌한 영어와, 마찬가지 사정의 한국인인 내가 만나 끙끙 앓는 순간도 많았지만ㅋㅋ 최대한 이해하려 노력하면서 배운 과정을 정리한다. 
<br/>
