...
나는 에러를 프로젝트, 주제 별로 맥락 위에서 이해하려고 했는데, 꼭 그렇지는 않고 에러만 에러끼리 모아 보는 것도 나름 의미가 있을 것 같았다!!  
그래서 단일 `string`만 받던 태그를 `string[  ]`의 멀티값도 받을 수 있도록 간단히 수정했다. 이렇게 해서 본 주제에도 에러가 담기는 한편, 별도의(에러픽스) 페이지에도 실리게 만들었다.  
<br/>
타입이 달라졌기 때문에 배열일 경우에 대한 내로잉을 추가해줘야 했으니-
<br/>
