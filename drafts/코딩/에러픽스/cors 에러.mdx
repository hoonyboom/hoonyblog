...
그래서 그큐엘 `endpoint`에 `www`를 달아줬고, `same-origin`이 아니어서 생긴 `CORS Preflight` 문제는 어이없게 해결됐다. 애초에 같은 출처이니 `Cross-Origin` 문제가 뜨면 안 됐던 셈이다.  

### ㅇㅋ 근데 preflight가 뭐였나요
클라이언트에서 다른 출처에 리소스를 요청할 때는 두 단계로 나뉘어서 진행된다. `preflight`는 이름에서 보듯이 예비비행, 일종의 입국심사 같은 건데,  
<br/>
...
<br/>
`credentials`는 기본적으로 `same-origin`에 한해서만 쿠키나 인증과 관련된 헤더를 요청에 담아 보낸다. 이때 와일드카드도 허용하는 출처로 인정해 버리는데, 의도치 않게 민감정보가 새어나갈 수 있다. 이때 `include`를 쓰면 좀 더 세밀한 증빙을 요구를 하게 된다.  
