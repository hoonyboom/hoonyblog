...
---
<HeadingNavigator />
으아악 지난 이틀 cors 때문에 아무것도 못했으니..  
언젠간 마주쳐야 했지만 그게 왜 이번이었는지ㅜㅜ 퍼진다 🫠🫠🫠   
...
그래서 그큐엘 `endpoint`에 `www`를 달아줬고, 그러자 `same-origin`이 아니라던 `Preflight` 문제는 어이없게 해결됐다. 애초에 같은 출처인데 `cross-origin`에서 문제를 찾았으니 개념이 없던 거다.  
...
<br/>
`credentials`는 기본적으로 `same-origin`에 한해서만 쿠키나 인증과 관련된 헤더를 요청에 담아 보낸다. 이때 와일드카드도 허용하는 출처로 인정해 버리는데, 의도치 않게 민감정보가 새어나갈 수 있다. 그럴 경우 `include`를 쓰면 좀 더 세밀한 증빙을 요구하게 된다.  
