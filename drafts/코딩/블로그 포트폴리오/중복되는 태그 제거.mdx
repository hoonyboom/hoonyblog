...
<br/>
### 근데 뭔가 아쉽다
왜냐면 내가 저런 식으로 필터를 걸었을 때, 사실 원했던 건 카테고리만이지 순서에 따라 뽑히고 말고가 결정된 오브젝트 전체는 아니다. 그런 id나 title 같은 거 사실 별 쓸데없기 때문이다..  
<br/>
...
<br/>
너희들..ㅎㅎ.. 중복값이 필요없는 params인데 중복돼 있었다. 이왕 본 김에 직접 잡아본다
<br/>
<br/>
### 또듀스.?
요새 부쩍 reduce를 많이 쓰는 것 같다. 인생은 물들노... 계속 써야 외우는 머리이기 때문에 한번 더 남용한다ㅎ
<br/>
...
```
<br/>
2. 중요한 건 역시 리턴인데, reduce의 매력은 이곳이다. *어떤 형태로 누적 데이터를 받을지 사용자 마음이라는 것!* 앞에서처럼 내가 필요한 건 오직 태그뿐이다. 때문에 리턴을 배열로, 안에는 `each.tags`라는 string 자료가 차곡차곡 쌓이게 만들었다.
<CH.Code>
...
<br/>
<br/>
\+ 22/09/25 update: <Lnk text={'Set 중복 제거'} href={'http://localhost:3000/posts/%EC%97%90%EB%9F%AC%20%EB%AA%A8%EC%9D%8C%EC%A7%91'} />
