---
title: 댓글 기능 추가 (Prisma+Postgres+GraphQL)
date: '2022-10-27'
categories: coding
tags: ["블로그 포트폴리오", "그큐엘"]
description: feat
...
기본적인 CRUD 기능을 연습하기 좋았고 그큐엘의 쿼리 메커니즘에도 한결 익숙해졌다.  
...
<br/>
아무튼 스키마와 `typeDefs`는 거의 같은 작업이다. 그래서 사실 좀 재미는 없었고, 제일 재밌는 부분은 역시 실제 일을 수행하게 되는 `resolvers`였다. `prisma` 메써드는 이 안에서 활용된다. 어떤 기능들이 있는지만 알면 타입스크립트에 의해 자동완성 기능이 지원되기 때문에 크게 어려운 건 없을 것 같았다. 물론 그거랑 "아니 왜 안돼?"는 별개의 문제입니다.
<br/>
<br/>
