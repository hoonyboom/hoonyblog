...
Next 공식 튜토리얼을 따라갈 때, 하나의 폴더를 대상으로만 `posts`를 읽히게 만들어놓은 탓이다. 지금은 이 폴더 안에 .mdx 외의 파일이 들어가면 오류를 던지고 있다. 용납될까? 안된다 🫥  
<br/>
그래서 오늘은 계층화(= 세분화, 하위폴더화)를 시켜보려 한다.
...
참고한 링크[^1]를 보면 경로를 매개변수로 받아 모든 폴더를 탐색하고 있다. 내부의 로직은  
   1. 파일일 경우 리턴  
