...
---
<HeadingNavigator />
내가 mdx 블로그를 좋아하는 이유 중 하나가 고립이다.  
글을 브라우저에서 쓰느냐 안 쓰느냐는 톤에 영향을 준다고 생각하는데, 로컬에서 글을 쓰면 좀더 자신과 글감에 집중하는 느낌을 받을 수 있다.
<br/>
아무튼 포스팅이 늘어나면서 파일 관리가 점점 힘들어지게 됐다. 처음 Next 공식 튜토리얼을 따라갈 때 하나의 폴더를 대상으로만 `posts`를 읽히게 만들어놨기 때문이다. 이거 된다 안된다? 당연히 안된다 🫥  
그래서 오늘은 이를 계층화(= 세분화, 하위폴더화) 시키려고 한다
...
<br/>
코드는 이전보다 좀더 복잡해져서, 따로 함수로 빼두는 게 맞는 것처럼 보였다. 때마침 읽는 마틴 파울러에서 나오는 이런 말 나왔는데 [^2]
...
지금 상황에 적절한 말처럼 들렸다. 항상 그렇지만 무슨 말이든 머리로 주억거리는 것보단 내 식대로 해보고 이해하는 게 더 깊고 오래 남는다. 없는 기억보단 엉터리여도 있는 게 낫다. 안 그런가? 그래서 우선 함수에 칼을 댔다.     
<br/>
`getSortedPostsData()` 함수는 데이터를 구함과 동시에 정렬하고 있는데, 여기서 데이터 구하는 부분을 `getAllFiles()` 함수로 따로 떼낸다
<CH.Scrollycoding>
*getAllFiles(  )*  
...
---
*reduce 간단 설명*  
첫번째 콜백에서 계산된 리턴값이 두번째, 세번째, ...마지막까지 넘어간다.
```ts posts.ts
Array.reduce() 간단 설명
const getAllFiles = (dir: string) => {
  const folderNames = fs.readdirSync(dir);
...
---
*전체적인 로직*  
폴더를 만나면 재귀함수가 되게끔, 파일을 만나면 {} 형태의 데이터를 리턴하게 만든다. 누적값은 배열안에 스프레드로 풀어 계속해서 이어지게끔 한다.
```ts posts.ts
const getAllFiles = (dir: string) => {
  const folderNames = fs.readdirSync(dir);
  const getAllFilesData = folderNames.reduce((allFiles, file: string) => {
