...
자매품으로 디바운스가 있는데, <q>디바운스</q>는 내가 지정한 시간만큼 움직임이 멈춰있을 때 함수가 호출된단 점에서 차이가 있다. 한마디로  
<br/>
- 쓰로틀: *쿨타임*마다 한번 실행  
- 디바운스: 멈추면 *카운트다운*, 그 안에 안 움직여야 실행
<br/>
정도로 보면 되겠다. 확실히 이해하고 넘어가기 위해 좀 더 예를 들어보자.  
>🚦 머리 위로 팔을 3시 방향에서 9시 방향으로 일정하게 휘두르려고 한다. 만약 이때 딜레이 간격을 3시간 간격으로 잡았다면 `throttle`은 12시에서 한번, 9시에 도착했을 때 한번, 이렇게 두번 실행될 것이다. 반면 `debounce`는 9시에 도착해 세시간을 멈춰야 명령을 한번 내뱉는다. (물론 12시에 도착해 세시간을 쉬고, 9시에 도착해 세시간을 쉬면 똑같이 12시와 9시가 찍힐 것이다.)  

이해가 됐을까? 개념을 잡았다면 쓰면 된다. <q>lodash</q>에서는 이를 아주 쉽게 쓸 수 있게 함수로 제공해 주고 있다. 그리고 우리는 심장소리처럼 스크롤바를 경청해야 한다. 따라서 디바운스가 아닌 쓰로틀을 걸어준다
...
### 간단설명
#### globalThis
ES11에 추가된 전역객체다. 통상 전역객체하면 `window`, `this`를 많이 쓴다. 하지만 환경에 따라 `self`, `global` 같은 녀석들을 쓰기도 했단다. 이때 globalThis는 환경과 무관하게 전역객체에 접근함으로써 혼돈을 정리한다. 코알못인 나에겐 기특한 녀석으로 보였기 때문에 window 대신 애용 중이다.  
<br/>
#### useRef()
...
<br/>
#### useMemo()
불필요한 리렌더링을 방지해주는 훅이다. `[  ]` 안에 의존성으로 입력한 값이 변화할 때만 움직이는 엉덩이 무거운 콜백 함수다. 여기서는 [beforeScrollY]를 걸어주어 스크롤값이 변할 때만 실행되게 했다. 메모리를 사용해서 계산값을 붙들고 있기 때문에 적절히 쓰면 성능 향상을 기대할 수 있다고 한다. 어떻게 보면 useEffect와도, useCallback과도 겹치는 부분이 있는데 그건 다음에 정리해보려고 한다.  
