...
리액트는 훅이 실행된 순서대로 index를 부여해 리렌더링 시에도 이를 참조, 걸려 있던 훅들을 정확히 불러낸다. 조건문에 의해 훅이 누락될시 인덱스가 밀리기 때문에 이를 금지하고 있다.
...
4. 함수 컴퍼넌트에서 초기값은 매번 로드된다. 그러니 연산 비용이 큰 초기값이라면 콜백을 줘 최초 렌더에만 실행되게(`lazy initializer`) 코드를 구성한다.  
```tsx example.tsx
function App(){
