...
리액트에 관해선 선배들이 알려준 대로, 일종의 공식처럼 따르는 부분이 많다. 그래서 아주 쉬운 생각도 보지 못하고 지나친 때가 많았음을 고백해둔다. 이 지면은 속죄를 위해 마련했다. 두루뭉술 지나갔던 개념들을, 엄청나진 않아도 적어도 나의 무지를 드러내는 작은 확대경으로 살펴보고자 한다.  
<br/>
...
### Hook's Ordering
리액트에서는 훅의 순서가 중요하다. `import`가 모듈의 최상단에 와야만 하는 것처럼, 리액트 훅은 컴퍼넌트 렌더링 코드의 최상단에 위치해야만 한다. 이를 무시하면 화면에서 에러를 띄우는데 그래서 아마 좋든 싫든 타성적으로 외우게 되는 부분 중 하나일 거다.  
<Img src='/images/2023/react/훅스는 최상위에.webp' />
> Hooks can only be called at the top level of components or your own Hooks.  

<br/>
이유를 묻기 앞서 다시 한번 생각해보았다. `useState`는 컴퍼넌트 내부에서 선언되지만 실제 그 값은 외부에 저장된다. 그래서 컴퍼넌트가 깜빡, 리렌더링 돼도 변화된 값은 함수 바깥에 저장되어 있기에 안전하게 보관, 불러올 수 있다는 것까진 나도 알았다.  
<br/>
...
<br/>
리액트 모듈 안의 코드와 세부적으로는 차이가 있겠지만 그 논리는 유사하다(이 예시코드가 리액트 팀에서 가져온 거다).  
코드를 훑으면서 과하리 만큼 주석을 남겼기 때문에 이를 중언부언하는 것은 낭비일 것 같고, 대신 먼 훗날 내가 봐서 내가 이해할 수 있게 짧은 요약으로 갈무리한다.
...
`updateDOM()`에 실행되면서 인덱스는 `0`으로 초기화된다(예시의 아래 포커스 부분).  
<br/>
함수 컴퍼넌트가 재실행 되면서
<br/>
