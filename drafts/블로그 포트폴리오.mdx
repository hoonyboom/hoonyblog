...
ㄱ. Lighthouse를 통해 사이트의 성능을 점검했다. 감점 요소가 무엇인지, 이유와 참조 링크를 자세히 알려 준다. 부랴부랴 수정해서 초록불 띠워버리기 
<br/>
![seo-test](/images/seo-test.png)
<br/>
ㄴ. 마크다운에 리액트를 불러올 때 매번 모듈을 불러오는 건 번거롭다. 그럴 땐 이렇게 커스텀 스니펫을 짜두면 시간을 아낄 수 있다.
```json markdown.json
...
<br/>
하지만 이 때문에 아직 호환이 안 되는 부분도 분명 있었다. 여기서는 tailwind 요소를 prettier 정렬에 반영하는 과정에서 오류가 발생했다. prettier는 자신의 plugin들을 찾을 때 현재로선 node_modules만을 색인하는데 yarn berry는 node_modules를 가지지 않기에 그를 직접 찾아 내보내야 했다.
