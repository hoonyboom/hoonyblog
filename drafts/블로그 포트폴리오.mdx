...
<br/>
ㄴ. 마크다운에 모듈을 매번 일일이 작성하는 게 번거로웠다. 이렇게 커스텀 스니펫을 짜두면 시간을 아낄 수 있다.
```json markdown.json
{
  "mdx": {
    "prefix": "@mdx", // 단축어는 @mdx로
    "body": [ // body 안에 코드를 작성. 한줄을 기준으로 ""에 담는다.
      "import { useState, useEffect } from 'react';",
      "",
      "export default function ${1:/*이름*/}({MdxComponents}) {",
      "  const { LINK, IMAGE, Notation, Youtube } = MdxComponents;",
      "  return (",
      "    <>",
      "      ${2:/*Code here*/}",
      "    </>",
...
<br/>
하지만 이 때문에 아직 호환이 안 되는 부분도 분명 있었다. 여기서는 tailwind 요소를 prettier 정렬에 반영하는 과정에서 오류가 발생했다. prettier는 자신의 plugin들을 찾을 때 node_modules만을 색인하기 때문에, yarn berry에선 그를 직접 내보내줘야 한다.
