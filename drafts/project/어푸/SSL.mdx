...
### 자동화 설정
인증서 발급은 끝났지만 여기에는 치명적인 문제가 있다. 무료 인증라서 그런가 3개월마다 재발급을 해줘야 한다는 것ㅋㅋ 말이야 빵구야. 다행히 모두의 염원처럼 자동화할 수 있는 방법이 있고 유닉스 계열 스케쥴러인 `cron`을 사용한다.

먼저 갱신을 요청하는 스크립트를 만든다. 그리고 이 스크립트를 주기적으로 실행하게 cron에 등록하면 된다.
