...
일단 지금 할일은 어떤 불명의 원인으로 잠긴 방화벽을 해체하는 것이다. 방화벽이 해체된다면 인스턴스에 평소처럼 접근할 수 있을 것이고, 추가 조치를 취할 수 있다.  
방화벽을 해제하자.
...
마지막으론 인스턴스를 reboot 하면서 홈페이지에 작성했던 스크립트를 삭제해주어 충돌을 방지했다. 잠긴 이유는 보안그룹에서 포트 22가 열려 있는 것과 allow 22로 명시해주는 것은 다르다는 설명으로 추측하고 있다.
<br/>
<br/>
---
### 부록
