...
### 이메일, 유료인가요?
프론트엔드만 할 적엔 메일을 보내는 서비스가 유료라고 생각했다. `Brevo`나 기타 등등 SMTP 서비스들을 사용한 적이 있는데, 무료 사용량 제한이 있어 여간 찝찝한 게 아니었다. 가난한 개발자는 아낄 수 있는 데서는 아껴야 한다..  
그런데 `nodemailer`를 사용하면 무료로 메일을 보낼 수 있었다. 유료 팀 서비스인 `Workspace`를 사용할 것도 없고, 그냥 팀의 관리자가 대표로 계정을 생성하여 그를 오피셜 아이디로 사용하면 됐다. 게다가 참고할 레퍼런스도 많다. 유레카!
...
유저에게 보내지는 메일은 `html` 프로퍼티에 담기는데, 직접 템플릿을 만들었다. 실제 메일은 다음처럼 보내지는데
<Img src='/images/2024/nodemailer1.webp' />
<figcaption>네이버 메일</figcaption>

<br/>
네이버부터 확인한 터라 이대로 끝났다 싶었는데, 이슈가 생겼다. 네이버에서는 문제없던 템플릿이 구글에서는 망가져서 보였던 것. 찾아보니 `Gmail`에서는 `flexbox` CSS를 지원하지 않는다고 한다.  
...
역시나 `Nest.js`는 필요하다 싶은 건 다 주고 있고(진짜.. 감사합니다!!) `Passport` 모듈 또한 제공하고 있다. 라이브러리 `passport-kakao`, `passport-naver` 또한 나와 있는 상황이어서 이를 쉽게 확장하여 OAuth를 구현할 수 있다. 구현방식은 똑같기 때문에 하나를 할 줄 아니 다음은 쉬워졌다.  
