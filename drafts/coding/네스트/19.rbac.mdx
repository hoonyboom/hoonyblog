...
서비스를 운영하면 사실 모든 부분을 비공개로, 폐쇄적으로 운영하지 않는다. 인스타그램이나 X처럼 로그인하지 않아도 어느 정도 틈을 두어 맛보기를 제공한다. 이렇듯 로그인 여부로, 혹은 롤을 기반으로 무엇을 보여주고 무엇을 숨길지 나누는 케이스가 일반적이고 이를 통칭 <q>알백(RBAC)</q>이라 부른다.
...
이전에 구현한 데코레이터는 `createParamDecorator`를 사용해 컨트롤러에서 사용되는 데코레이터를 만드는 거였다. 하지만 지금 구현하는 RBAC은 파라미터 데코레이터가 아니라 그 이전 시점에서 유저가 가진 직책을 검증할 가드다.  
NestJS에서 데코레이터는 메타데이터(`reflect-matadata`)를 기반으로 작동하는 기술이다[^1]. `@nestjs/common`에서는 메타데이터를 설정하는 `SetMetadata` 함수를 제공하고 있기 때문에, 손쉽게 알백 전용 데코레이터를 만들 수 있다.
...
하지만 메타데이터를 등록했다고 해서 이것만으로 뭔가 되는 게 아니다. 당연하다. 메타데이터가 셋팅됐으니 이를 읽어내서 원하는 작업을 실행하는 로직을 만들어야 한다.

우리가 원하는 작업은 `ADMIN`이냐 아니냐를 따지는 작업이다. 즉 `@Roles` 데코레이션이 활성화된 라우트에 대해 `Guard` 로직이 실행돼야 한다.
...
providers: [
  ...
  ...
```
이렇게 하면 어떤 API든 액세스토큰 가드의 검증 대상이 된다. 토큰을 확인할 수 없는 경우에는 에러를 낸다. 그리고 우리는 이런 완전한 폐쇄를 지양한다.   
...
메타데이터를 만드는 모습은 똑같다. 하지만 새로운 가드를 만드는 게 아니라 `BearerTokenGuard` 안에서 검증이 이루어지게 한다. 그 안에서 `isPublic`이 true로 확인되면 액세스 토큰 검증을 멈춘다.  
