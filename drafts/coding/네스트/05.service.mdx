...
왜냐면 NestJS가 주입 가능한 클래스의 인스턴스를 자동으로 생성해주기 때문이다. 이런 게 아마 프레임워크의 힘인 듯하다. IoC 컨테이너는 프레임워크에서 그 작업을 도맡아 처리한다. 개발자는 `@Injectable()` 데코레이터를 달아 이 클래스가 DI로 활용될 객체임을 알려주기만 하면 된다. 멋진 신세계!  
- IoC 컨테이너의 도움으로 객체의 라이프 사이클에 신경 쓰지 않아도 된다.
- 인스턴스의 생성과 폐기를 신경 쓰지 않고 비즈니스 로직에만 집중할 수 있다.
- 별도의 scope를 지정하지 않으면 `Singleton` 인스턴스로 생성되어 불필요한 중복, 낭비를 막는다.
...
- 프레임워크는 자신의 틀에서 요구되는 객체에만 관심이 있으며, 그 형식만 충족한다면 구체적인 내용(구현)에 대해서는 무관심하다.
...
<br/>
가령 위 사진의 B는 서비스 / A는 컨트롤러에 대입될 수 있다. 컨트롤러에서 서비스가 요구되는 순간, IoC 컨테이너에서 서비스=DI임을 식별하고 인스턴스를 생성➝주입해준다. NestJS는 cli로 모듈을 만들 때 이 패턴을 기본으로 쥐어준다. 하위 서비스를 직접 만들어 쓰더라도 `@Injectable()` 데코레이터를 붙이기만 하면 모듈에 등록하여 쓸 수 있다.
- `@Injectable()` 데코레이터는 해당 클래스를 DI로 활용하겠다고 프레임워크에 보내는 일종의 딱지다.
- 이 클래스가 어딘가에 주입되는 상황에 NestJS의 IoC 컨터이너가 해당 클래스의 인스턴스를 자동으로 생성해 넣어준다.
- 이렇게 의존성으로 들어가는 클래스를 더 일반적으로 부르는 말이 `Provider`다. NestJS에서는 서비스, 레포지토리, 미들웨어 등이 모두 Provider다.
