...
- 페이지 기반 페이지네이션
  - 페이지를 기준으로 데이터를 컷팅(offset)한다.
  - 요청을 보낼 때 원하는 데이터 개수와 몇 번째 페이지인지를 명시한다.
  - 페이지 숫자를 누르면 다음 페이지로 이동하는 UI로 구현된다.
  - 이동하는 도중 데이터가 추가되거나 삭제되는 경우 데이터의 누락이나 중복이 일어날 수 있다.
  - 알고리즘이 간단한 편이다.

하지만 스마트폰 보급과 함께 유행하고 있는 페이지네이션은 스크롤 기반의 페이지네이션이다. 스크롤을 내리며 바닥에 도달할 때까지 이른바 <q>무한 스크롤</q>을 구현하는 방식이다.
- 스크롤(커서) 기반 페이지네이션
...
사실 두 경우 모두 일상에서 자주 보는 형태다. 하나가 더 우월하다 아니다로 쉽게 나눌 수 없고, 서비스에 따라 적절한 방식을 택하는 게 좋다. 물론 공부하는 입장에서야 두 가지 방식을 다 챙겨야 하겠지만.  

그런데 까놓고 얘기하자. 솔직히 말해 이 구역의 주인은 스크롤 기반의 페이지네이션이라는 생각을 지울 수 없다. 통상 더 많은 실력과 경험을 요하기 때문이기도 하지만, 이를 구현할 줄 안다면 페이지 기반의 페이지네이션을 공존시키는 것은 그리 어려운 일이 아니기 때문이다.
...
그런데 이상하다. 아무 값도 받지 않았을 때 기본값을 주면 `class-validator`는 에러라고 인식한다. `@IsOptional`을 달았는데도 그렇다. `undefined`을 내보낼 작정이었는데 `number`가 반환됐기 때문이다. 그럴 땐 다음의 옵션을 `main.ts`에 추가한다.
```ts main.ts
app.useGlobalPipes(
  new ValidationPipe({
    transform: true,
...
- `enableImplicitConversion: true`를 추가하면 데코레이터에 기반하여 타입 변환을 자동으로 한다. 모든 쿼리는 `String` 타입인데, 이 옵션을 추가하면 `@IsNumber` 데코레이터만으로 타입이 `Number`로 자동 변환된다. 이 옵션을 쓰지 않으면 `@Type(()=>Number)`처럼 추가 데코레이터를 붙여야 같은 효과를 누릴 수 있다.
<br/>
...
### 메타데이터 작성
위 조회의 결과 응답으로 내보낼 데이터를 얻게 된다.  
하지만 이 데이터는 단순히 데이터일 뿐, 어떤 페이지네이션의 결과인지, 다음 페이지가 있는지 등의 정보가 없다. 즉 페이제네이션에 관한 메타 데이터가 없다. 이는 클라이언트로 응답할 때 데이터와 함께 꼭 같이 포함돼야 하는 값이다.

- 커서 페이지네이션의 메타데이터 구성
  - after: 현재 커서가 가리키고 있는 id값이 온다. 다음 요청의 기준으로 사용되는 값이다.
  - count: 현재 조회하여 응답하는 데이터의 수다. 이 값이 take보다 작다면 남은 데이터가 없다는 뜻이다.
  - next: 다음 요청으로 쓰일 url 주소를 담아보낸다. 이 덕분에 프론트엔드에서 일을 쉽게 할 수 있다. 다음 데이터가 없다면 null로 처리해 추가 요청을 방지한다(프론트엔드에서는 그에 맞는 UI를 띄우도록 한다).
- <b>로직 쪼개보기</b>
<CH.Scrollycoding>
...
이러면 기본적인 모양새는 충족한 페이지네이션 함수가 완성됐다. 여기서 기능을 좀더 보완하고, 나아가 일반화시키는 것이 개발자의 목적이겠지만 그게 결코 간단하지만은 않다... 그러니 다음 기회로 넘기고
포스트를 끝내기 전에 페이지 기반 페이지네이션의 메타데이터도 구성도 정리해보자. 앞서에 비해 훨씬 간단한 모양새를 취하고 있다.
...
[^2]: <Lnk text='class validator의 옵션들' href='https://velog.io/@arara90/class-validator-ValidateIf' />