...
<br/><br/><br/>
### 제네릭이 어떻게 짜여있길래?
이를 이해하기 위해선 다른 유틸 타입인 <a>Pick</a> 과 <a>Exclude</a> 의 쓰임을 먼저 알아야 했다. 내가 단박에 파악할 깜냥은 안 돼서 다른 분의 설명을 참고했고, 많은 도움이 됐다. 자세한 설명이라면 그분 걸 참고하는 게 나을 것 같다. 아무래도 나는 내가 적고 내가 이해하는 데 좀 더 주안을 두기 때문이다.  
<br/>
아무튼 말로 풀면 Omit은 Pick의 반대, 부정형이다. Pick이 제네릭을 통해 조건에 해당되는 값을 가져온다면 Omit은 그것을 제외한 모든 값을 들고 온다. 구조는 동일하다. 단지 <a>Pick`<`대상, 조건`>`</a>에서 '플러스'였던 조건을 '-조건'으로 바꾼 것이 Omit인 것. 그리고 그런 부정논리를 만들어주고 있는 녀석이 Exclude일 뿐이다.  
간략하게나마 이를 살펴보자
<br/>
<br/>
<hr/>
...
<a>keyof</a>는 오브젝트에서 키값을 뽑아내 유니언 타입으로 나열해주는 오퍼레이터다. Pick은 그의 도움을 받아 다음과 같이 이루어진다.
```ts sample.ts
