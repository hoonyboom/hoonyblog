...
당초 의도는 children을 생략하고 그 자리를 text와 css로 대신하는 거였다. 물론 그거랑 children이랑 작동이 별반 다를 거 없긴 하다. 근데 왜 바꾸냐고? 아니 css가 이상하게 들어가더라고요..ㅎㅎ;;   
그래서 어떻게 해야 할까?  
<br/>
### 방법은 Typescript의 'Omit' 💨💨
- 타입스크립트의 유틸타입 중 하나!
- 타입을 _`extends`_ 해올 때 <Note text={'특정키 값을 제외하고 상속'} /> 받는다
<br/>
공식문서 둘러볼 때 이놈은 필히 쓰겠다 싶더랬지ㅎ 사용법은 간단하다.
```ts Mdx....tsx mark=1[33:68]
interface NotationProps extends Omit<RoughNotationProps, "children"> {
  text: string;
  css?: string;
} // children을 제외하고 extends 해온다.
```
이러면 이제 children을 안 쓰거나 혹은 원래 의도대로 옵셔널하게 재정의할 수 있을 거시외다~  
끝.
<br/>
