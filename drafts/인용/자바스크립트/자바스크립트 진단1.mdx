...
프레임워크, 라이브러리... 다 좋지만 기본기가 부족하다는 생각이 들었다. 인강에서 배운 걸로 퉁 치고 넘어가기엔 취업 문턱이 만만치 않더라^^.. 〈Eloquent Javascript〉 읽으면서 기초를 다 잡으며, 모르는 걸 드러내기 위해 매일 객관식 퀴즈를 푼다.
</br>
각각의 포스팅은 열다섯 문제를 포함한다. 각 문제는 답이라고 생각한 코드와 그 생각의 이유를 다루며, 감으로 유추한 경우라 하더라도 이에 대한 코멘트를 남긴다. 오답이었을 경우 무엇이 잘못이었는지 짚고 정확한 접근을 기술한다.
</br>
문제의 출처는 `Vercel`의 <q>Lydia Hallie</q>다.
