...
프레임워크, 라이브러리... 다 좋지만 기본기가 부족하다는 생각이 들었다. 인강에서 배운 걸로 퉁 치고 넘어가기엔 취업 문턱이 만만치 않더라^^.. 〈Eloquent Javascript〉 읽으며 기초를 다지면서, 모르는 걸 드러내기 위해 매일 객관식 퀴즈를 풀려고 한다.
<br/>
각각의 포스팅은 열다섯 문제를 포함한다. 문제는 열흘치다. 각 문제는 답이라고 생각한 코드와 생각의 단서를 달고 있다. 감으로 유추한 경우라 하더라도 이에 대한 코멘트를 남길 것이다. 오답일 땐 무엇이 잘못이었는지 짚고, 바람직한 접근을 기술한다.
<br/>
문제의 저작자는 `Vercel`의 <q>Lydia Hallie</q>다.
