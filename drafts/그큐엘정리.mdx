...
### 후기
그큐엘의 장점은 앞에도 쓰여 있지만, 직접 써보면서 좀 더 데이터에 관심을 갖게 된 게 난 좀 큰 거 같다. 나 잘났다는 말은 절대 아니다아.. 어차피 프론트엔드도 interface 뽑으면서 들어오는 데이터에 대해 일상적으로 파악하기 때문이다.  
그럼 뭘까?  
내가 말하고 싶은 건 그 둘을 따로 처리하는 것의 부당함, <a>공들인 스키마와 또다시 작성되는 ts를 합치는 방식이 없을까</a>에 대한 관심이다(차라리 호기심이라고 해두자). 그래서 삽질을 좀 했다 💁🏽
<br/>
   1. type-graphql  
   일종의 type ORM이다. 자체 오퍼레이터를 활용하여 typeDefs를 작성한다. 그런데 솔직히 까놓고 얘기해도 될까? 처음부터 끝까지 별로였다. 일단 코드가 길어지고 resolvers에 가선 더 복잡해진다. 뭣보다 내 궁금증은 서버에서 공들여 짠 스키마를 클라이언트에서도 통용되게 할 수 있느냐였는데, 근데 그게 아니었다는 데서 크리티컬하게 느껴졌다. 다신 안 다룰 거 같아 내용도 까먹어벌임;
...
<br/>
   ...
   그큐엘 스키마를 타입스크립트 형태로 추출해준다. 내가 찾던 기능일까? 맞다. 하지만 문제는 추출된 타입을 읽기가 우리 사람들이 쓴 것과 달리 꽤 난해하다는 것이다. 편하기는 한데, 타입스크립트를 다루는 동료가 봐도 파악이 어렵다면 이걸 쓰는 게 맞냐는 생각.. 아래는 공홈의 예다. 물론 조금만 애쓰면 이해가 되지만, 그런 조금의 노력도 동료에겐 부담일 수 있기에.. 아무래도 몇분 걸려 직접 쓰는 게 나을 거 같다.
...
<br/>
4. tRpc (+ Prisma)  
와,, 이거 딱 내가 찾던 거.. 너무 신박했다. 별다른 추출이나 변환 없이도 서버의 스키마를 클라이언트에서 <a>유추해낸다</a>. 타입이 저절로 유추되기에 어떤 타입스크립트의 작성도 불필요하다. 서버에서의 설계는 한층 더 복잡해졌지만ㅜ.  
