...
### 후기
그큐엘의 장점은 앞에도 쓰여 있지만, 난 데이터에 관심을 갖게 된 게 좀 큰 거 같다. 근데 핀트가 좀 다른가? 내가 관심을 가진 쪽은 <a>그토록 공들여 짠 스키마를 또다시 ts로 작성해야 하는 일의 부당함</a>쪽이다. 그래서 이를 통합하는 방식이 없을까 찾아보게 되었다.
<br/>
   1. type-graphql  
   자체 오퍼레이터를 활용하여 typeDefs를 작성한다. 그런데 까놓고 얘기하면 처음부터 끝까지 모든 게 별로였다. 일단 코드가 길어지고 resolvers에 가선 더 복잡해짐. 뭣보다 내 궁금증은 서버에서 공들여 짠 스키마를 클라이언트에서도 통용되게 할 수 있느냐였는데, 그게 아니었다는 게 크리티컬했다. 다신 안 다룰 거 같아 내용도 까먹음   
