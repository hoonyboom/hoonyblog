...
물론 어떤 걸 써도 Graphql 서버는 <Note>typeDefs</Note>와 <Note>resolvers</Note>로 구성된 스키마의 설계를 요구한다.  
<CH.Section>
<CH.Code>
```ts yoga-graphql.ts
...
그큐엘 사용자는 데이터의 구성=shape를 먼저 서버에 기술해야 한다(즉 꿰고 있어야 한단 말)  
그래서 클라이언트에서 필요한 쿼리를 선별적으로 요청할 수 있는 것이다.  
<br/>
어떻게 작성하는지 예를 살펴보자  
...
### 후기
그큐엘의 장점은 앞에도 쓰여 있지만, 직접 써보면서 좀 더 데이터에 관심을 갖게 된 게 난 좀 큰 거 같다. 나 잘났다는 말은 결코 아니다아.. 어차피 프론트엔드도 interface 뽑으면서 들어오는 데이터에 대해 일상적으로 파악하기 때문이다.  
그럼 뭘까?  
내가 말하고 싶은 건 그 둘을 따로 처리하는 것의 부당함, <a>공들인 스키마와 또다시 작성되는 ts를 합치는 방식이 없을까</a>에 대한 관심인데, 차라리 호기심이라고 해두자. 그래서 삽질을 좀 했다 💁🏽
<br/>
   1. type-graphql  
...
<br/>
   2. Prisma + Nexus  
   이건 좀 버거웠다. 과정만 간단히 추리면 먼저 1) 프리즈마로 Model 스키마를 짠다. 2) 그럼 Nexus는 이 스키마를 Graphql 스키마로 변환해주는데, makeSchema 훅을 이용해 이를 그큐엘 서버로 넘기면 끝이다. 하.. 나도 이렇게 말하는 내가 싫다ㅎㅎ.. 아무튼 이것도 클라이언트와는 무관한 일이었지만 그래도 Prisma에 대해선 호감? 호기심이랄 게 생겨서 공부해 보고 싶다. 일단 type-graphql 보단 백만배 나았음.
<br/>
   3. Graphql Code Generator  
   그큐엘 스키마를 타입스크립트 형태로 추출해준다. 내가 찾던 기능일까? 맞다. 하지만 문제는 추출된 타입이 우리 사람들이 쓴 것과 달리 읽기 꽤 난해하다는 것이다. 편하기는 한데, 타입스크립트를 다루는 동료가 봐도 파악이 어렵다면 이걸 쓰는 게 맞냐는 생각.. 아래는 공홈의 예다. 물론 조금만 애쓰면 이해가 되지만, 그런 조금의 노력도 동료에겐 부담일 수 있기에.. 아무래도 몇분 걸려 직접 쓰는 게 나을 거 같다.
...
<br/>
4. tRpc (+ Prisma)  
와,, 이거 딱 내가 찾던 거.. 너무 신박했다. 별다른 추출이나 변환 없이도 서버의 스키마를 클라이언트에서 <a>유추해낸다</a>. 타입이 저절로 유추되기에 어떤 타입스크립트의 작성도 불필요하다. 서버에서의 설계는 한층 더 복잡해지긴 했지만ㅜ  
