...
<br/>
예시는 노마드코더의 그큐엘 강의 + 그큐엘 공식문서에서 뽑았다. 객관성은 없고 내가 쓰면서 이해하려고 적은 부분이 큼ㅋㅋ  
<br/>
...
### 후기
그큐엘의 장점은 앞에도 쓰여 있지만, 난 데이터에 관심을 갖게 된 게 좀 큰 거 같다. 근데 핀트가 좀 다른가? 내가 관심을 가진 건 <a>그토록 공들여 짠 스키마를 또다시 ts로 작성해야 하는 일의 부당함</a> 쪽이었다. 왜 똑같은 일을 두번해야 하지? 그래서 이를 통합하는 방식이 없을까 찾아보게 되었다.
<br/>
   1. type-graphql  
...
<br/>
   2. Prisma + Nexus  
   이건 좀 버거웠다. 과정만 간단히 추리면 1) 먼저 프리즈마로 Model 스키마를 짠다. 2) Nexus는 이 스키마를 Graphql 스키마로 변환한다. 그리고 makeSchema 훅을 사용해 이를 그큐엘 서버로 넘겨준다. 끝. 이게 다다. 물론 이것도 클라이언트와는 무관한 일이긴 했지만 그래도 Prisma에 대해선 호감? 호기심이랄 게 생겨서 완전 허탕은 아니었다. 일단 type-graphql 보단 백만배 나았음.
<br/>
   3. Graphql Code Generator  
   그큐엘 스키마를 타입스크립트 형태로 변환해준다. 몇 줄의 터미널 명령으로. 하지만 문제는 추출된 타입이 사람이 쓴 것과 달리 읽기가 좀 난해하다는 데 있다. 분명 편하기는 한데, 타입스크립트를 다루는 동료가 봐도 이게 단박에 이해가 안 되면 쓰는 게 맞냐는 생각.. 아래는 공홈의 예시다. 물론 조금만 애쓰면 이해가 되지만, 그런 조금의 노력도 동료에겐 부담일 수 있기에..  
...
하지만 회사에서처럼 분담이 이루어져 있는 경우, 백엔드에서 한번 프론트에서 한번 데이터의 타입을 더블체크하는 게 안정성에 더 도움이 될지 모르겠다. 나도 편하고 백엔더도 편하고. 속도가 더 빠르기도 하다. 그치만 혼자 할 때는 이게 옳은 방향이라고 느끼는데 어떡할까? 그래서 tRpc는 계속 공부하면서 조만간 따로 포스팅해보려고 한다.! 쾅쾅  
<br/>
