...
왜냐면 클라이언트에서 쿼리를 요청할 때나 또다른 기둥인 resolvers에서도 이 typeDefs를 기반해 로직을 수행하기 때문이다.  
<br/>
예시는 노마드코더의 그큐엘 강의 + 그큐엘 공식문서에서 뽑았다. 나중에 틈틈이 보려고 적어둔다. 
<br/>
...
2. **Mutation 타입**
   ...
   - 하지만 역할을 기준으로 한 개념적인 구분일 뿐, 실은 Query 타입과 기능적으로 동일하다. 그래도 약속을 준수하자!
...
### resolvers
   ...
   - 왜냐면 쿼리나 뮤테이션, 섭스크립션이 무슨 일을 행할지 다 리졸버에서 풀어지기 때문이다. 가령 쿼리를 통해 GET 요청을 했다면, 그 쿼리가 내뱉을 리턴값은(DB든 외부 API든) 명세서는 리졸버에서 찍힌다.
   - 이때 멋진 것이 타입 간의 상호참조다. 클라에서 쿼리에 뭔가를 GET 요청했을 때, 요청값이 단순 그 쿼리의 리졸버로 충족이 되지 않는 상황이라면, 그큐엘은 그 쿼리에 연관된 다른 타입의 리졸버를 참고해 요청된 작업을 수행한다.
   ```ts resolvers.ts
   const resolvers = {
      Query: {
...
### 후기
그큐엘의 장점은 앞에도 쓰여 있지만, 난 데이터에 관심을 갖게 된 게 좀 큰 거 같다. 근데 핀트가 좀 다른가? 내가 관심을 가진 건 <a>그토록 공들여 짠 스키마를 또다시 ts로 작성해야 하는 일의 부당함</a> 쪽이었다. 왜 일을 두번해야 하지? 그래서 이를 통합하는 방식이 없을까 찾아보게 되었다.
<br/>
   1. type-graphql  
   자체 오퍼레이터를 활용하여 typeDefs를 작성한다. 그런데 까놓고 얘기할까? 처음부터 끝까지 모든 게 구렸다. 일단 코드가 길어지고 resolvers에 가선 더 복잡해짐. 뭣보다 내 궁금증은 서버에서 공들여 짠 스키마를 클라이언트에서도 통용되게 할 수 있느냐였는데, 그게 아니었다는 게 크리티컬했다.  
   <Img src='/images/2022/summer/type-graphql.png' />
<br/>
   2. Prisma + Nexus  
   이건 좀 버거웠다. 과정만 간단히 추리면 1) 먼저 프리즈마로 Model 스키마를 짠다. 2) Nexus는 이 스키마를 Graphql 스키마로 변환한다. 그리고 makeSchema 훅을 사용해 이를 그큐엘 서버로 넘겨준다. 그럼 됐다. 이게 끝이다. 결과적으로 이것도 클라이언트와는 무관한 일이긴 했지만 그래도 Prisma에 대해선 호감? 호기심이랄 게 생겨서 완전 허탕은 아니었다. 일단 type-graphql 보단 백만배 나았음.
<br/>
   3. Graphql Code Generator  
   그큐엘 스키마를 타입스크립트 형태로 변환해준다. 문제는 추출된 타입이 사람이 쓴 것과 달리 읽기가 좀 난해하다는 거다. 진짜 편하기는 한데, 타입스크립트를 다루는 동료가 봐도 단박에 이해가 안 되면 이걸 쓰는 게 맞냐는 생각.. 아래는 공홈의 예시다. 물론 조금만 애쓰면 이해가 되지만, 그런 조금의 노력이 동료에겐 부담일 수 있기에 꺼려진다.  
...
이거다! 서버의 스키마를 별다른 추출이나 변환 없이도 클라이언트에서 <a>유추해낸다</a>. 서버의 타입이 클라에서도 통용되기에 어떤 타입스크립트의 작성도 불필요하다. 물론 서버에서의 설계는 한층(체감상 세층은) 더 복잡해지지만  
하지만 회사에서처럼 분담이 이루어져 있는 경우, 백엔드에서 한번 프론트에서 한번 데이터의 타입을 더블체크하는 게 안정성에 더 도움이 될지 모르겠다. 나도 편하고 백엔더도 편하고. 맞는 말이다. 그치만 혼자 할 때는 이게 옳은 방향이라고 느끼는데 어떡하나.. 그래서 tRpc는 계속 공부하면서 조만간 따로 포스팅해보려고 함.! 쾅쾅  
<br/>
