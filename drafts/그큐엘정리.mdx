...
### resolvers
   ...
   - 왜냐면 쿼리나 뮤테이션, 섭스크립션이 무슨 일을 행할지 다 리졸버에서 풀어지기 때문이다. 가령 쿼리를 통해 GET 요청을 했다면, 그 쿼리가 내뱉을 리턴값은(DB든 외부 API든) 리졸버에서 명세서가 찍힌다.
...
### 후기
그큐엘의 장점은 앞에도 쓰여 있지만, 난 데이터에 관심을 갖게 된 게 좀 큰 거 같다. 근데 핀트가 좀 다른가? 내가 관심을 가진 쪽은 <a>그토록 공들여 짠 스키마를 또다시 ts로 작성해야 하는 일의 부당함</a>쪽이었고, 그래서 이를 통합하는 방식이 없을까 찾아보게 되었다.
<br/>
   1. type-graphql  
   자체 오퍼레이터를 활용하여 typeDefs를 작성한다. 그런데 첨부터 솔직히 까놓고 얘기하면 모든 게 별로였다. 일단 코드가 길어지고 resolvers에 가선 더 복잡해짐. 뭣보다 내 궁금증은 서버에서 공들여 짠 스키마를 클라이언트에서도 통용되게 할 수 있느냐였는데, 그게 아니었다는 데서 크리티컬했다. 다신 안 다룰 거 같아 내용도 까먹어버렸다.. ㅅㅂ 
   <Img src='/images/2022/summer/type-graphql.png' />
<br/>
   2. Prisma + Nexus  
   이건 좀 버거웠다. 과정만 간단히 추리면 먼저 1) 프리즈마로 Model 스키마를 짠다. 2) Nexus는 이 스키마를 Graphql 스키마로 변환해주는데, makeSchema 훅을 이용해 이를 그큐엘 서버로 넘기면 끝이다. 아무튼 이것도 클라이언트와는 무관한 일이긴 했지만 그래도 Prisma에 대해선 호감? 호기심이랄 게 생겨서 공부해 보고 싶은 마음이 들었음. 일단 type-graphql 보단 백만배 나았다.
<br/>
   3. Graphql Code Generator  
   그큐엘 스키마를 타입스크립트 형태로 변환해준다. 문제는 추출된 타입이 사람이 쓴 것과 달리 읽기가 좀 난해하다는 것이다. 편하기는 한데, 타입스크립트를 다루는 동료가 봐도 단박에 안 읽히면 이걸 쓰는 게 맞냐는 생각.. 아래는 공홈의 예시다. 물론 조금만 애쓰면 이해가 되지만, 그런 조금의 노력도 동료에겐 부담일 수 있기에 꺼려진다.  
   아무래도 몇분 걸려 직접 쓰는 게 나을 거 같다는 생각이다.
...
<br/>
4. tRpc (+ Prisma)  
이거다! 서버의 스키마를 별다른 추출이나 변환 없이도 클라이언트에서 <a>유추해낸다</a>. 서버의 타입이 클라에서도 통용되기에 어떤 타입스크립트의 작성도 불필요하다. 물론 서버에서의 설계는 한층(체감상 세층) 더 복잡해지긴 하지만..  
그래도 tRpc는 계속 공부하면서 조만간 따로 포스팅해보려고 한다.! 쾅쾅
