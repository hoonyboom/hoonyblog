...
그큐엘 사용자는 데이터의 구성=shape를 먼저 서버에 기술해야 하는데(꿰고 있어야 한단 말)  
typeDefs는 일종의 지도가 된다.
클라이언트에서 쿼리를 요청할 때나 또다른 기둥인 resolvers에서도 이 typeDefs를 기반하여 로직을 수행한다.
<br/>
...
### resolvers
   ...
   - 왜냐면 쿼리나 뮤테이션, 섭스크립션이 무슨 일을 행할지 다 리졸버에서 풀어주기 때문이다. 가령 쿼리를 통해 GET 요청을 했다면, 그 쿼리는 자신과 대응하는 리졸버를 호출/참고한다. 거기서 DB든 외부 API든 특정값을 리턴하도록 코드가 만들어짐.
...
### 후기
그큐엘의 장점은 앞에도 쓰여 있지만, 누가 뭐래도 직접 써보면서 데이터에 관심을 갖게 된 게 난 좀 큰 거 같다. 나 잘났다는 말은 아니고ㅎ 어차피 프론트엔드들도 interface 뽑으면서 데이터에 대해 다들 일상적으로 파악하기 때문이다.  
그럼 뭘까?  
내가 말하고 싶은 건 그 둘을 따로 처리하는 것의 부당함, <a>공들인 스키마와 또다시 작성되는 ts를 합치는 방식이 없을까</a>에 대한 관심이다(차라리 호기심이라고 해두자). 그래서 삽질을 좀 했는데.. 뚜둔 💁🏽
<br/>
   1. type-graphql  
